...
@app.route('/elves/<int:elf_id>', methods=['PUT'])
...
def index_schedules():
    ...
    if args['login'] and args['password']:
        hash_password = md5(args['password'].encode()).hexdigest() 
        cursor = db.execute(
            """SELECT schedules.id, elves.login as elf, wish_id, done, done_at 
            FROM schedules
            LEFT JOIN elves on elves.id = schedules.elf_id
            WHERE login = ? and password = ?""",
            [args['login'], hash_password]
        )
...
@app.route('/schedules/<int:id>/done', methods=['PUT'])
def update_schedules(id):
    db = get_db()
    cursor = db.execute(""" UPDATE schedules
                            SET done = True
                            WHERE id = ?""", [id])
    cursor = db.execute(""" UPDATE schedules
